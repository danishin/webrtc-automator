...
object Fetch extends Helper {
  def run(platform: Platform): Program[Unit] = platform match {
    case Platform.IOS =>
      for {
//        _ <- modifyEnv(_.copy(cwd = root.webrtc.path))
        _ <- echo("Clone depot_tools")
        _ <- shell("git", "clone", "https://chromium.googlesource.com/chromium/tools/depot_tools.git", root.webrtc)

        _ <- modifyEnv(_.copy(envVars = List("PATH" -> appendToEnv("PATH", root.webrtc.depot_tools), "GYP_DEFINES" -> "OS=ios")))
