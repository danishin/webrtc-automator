...
private object RTCTypesParser extends JavaTokenParsers {
  ...
  def apply(reader: Reader): List[RTCType] = parse(expr, reader) match {
    case Success(l, rest) => l
...
object Assemble extends Helper {
  def run(archs: List[Platform#Architecture]): Program[Unit] = archs.head.platform match {
    case Platform.IOS => for {
      ...
      * */
      _ <- file.write(root.output.`WebRTCiOS.framework`.Versions.A.Headers("RTCTypes.h"), {
        val rtcTypes = RTCTypesParser(new FileReader(webrtcHeaderFiles.find(_.getName == "RTCTypes.h").get))
        List(
